...
Psi_Simulation<-function(iter=5000, Psi=0.5, Max=200, N=12){
	...
	alpha_Psi=0.10;
	sigma_nomial_Psi=sigma_nomial;
	delta_nomial_Psi=delta_nomial;
	
	sample_size_Psi<-function(sample_size=n){
		n1=as.integer(n*pivotal_dtr);			n2=n-n1;
		...
		G1=qt(p=1-alpha_Psi/2, df=n_star-2)/sqrt(n_star-2);
		a_U1=(U-delta_nomial_Psi)/sqrt( sigma_nomial_Psi/time1 );
		a_L1=(L-delta_nomial_Psi)/sqrt( sigma_nomial_Psi/time1 );
		r_U1=max(a_U1, (Psi*U-delta_nomial_Psi)/sqrt( sigma_nomial_Psi/time1 ) );
		r_L1=min(a_L1, (Psi*L-delta_nomial_Psi)/sqrt( sigma_nomial_Psi/time1 ) );
		
		return(integrate( function(x){
							G1=qt(p=1-alpha_Psi/2, df=n_star-2)/sqrt(n_star-2);
							a_U1=(U-delta_nomial_Psi)/sqrt( sigma_nomial_Psi/time1 )-G1*sqrt(x);
							a_L1=(L-delta_nomial_Psi)/sqrt( sigma_nomial_Psi/time1 )+G1*sqrt(x);
							r_U1=max(a_U1, (Psi*U-delta_nomial_Psi)/sqrt( sigma_nomial_Psi/time1 )+G1*sqrt(x) );
							r_L1=min(a_L1, (Psi*L-delta_nomial_Psi)/sqrt( sigma_nomial_Psi/time1 )-G1*sqrt(x) );
							
							XX=pnorm(a_U1)-pnorm(a_L1)+integrate(function(x1){
										G2=qt(p=1-alpha_Psi/2, df=n+n_star-2);
										a_U=(U-delta_nomial_Psi)/sqrt( sigma_nomial_Psi/time2 )-G2;
										a_L=(L-delta_nomial_Psi)/sqrt( sigma_nomial_Psi/time2 )+G2;
										
										dnorm(x1)*(pnorm(a_U*(n+n_star)/n*sqrt(time3/time2)-sqrt(n_star^2*time2/(n^2*time1) )*x1) - 
													pnorm(a_L*(n+n_star)/n*sqrt(time3/time2)-sqrt(n_star^2*time2/(n^2*time1) )*x1) )
									},
									lower=r_L1, upper=a_L1 )$value;
							return(XX*dchisq(x, df=n_star-2));}, lower=sample_size/2, upper=sample_size*2)$value );
	}
	
	N0=12;
	while(sample_size_Psi(sample_size = N0)<=0.8 & N0<200){
		N0=N0+2;
	}
	print(N0);
	
	#N1=as.integer(sigma_nomial*(qnorm(1-alpha_local/2)+qnorm(0.5+beta*pilot_power/2))^2*1/U^2 )+40;
	N11=as.integer(N1*s_dtr[2]);N12=N1-N11;
	
	N2=as.integer((1-s_dtr[1])/s_dtr[1]*1*N1);
	N21=as.integer(N2*s_dtr[2]);N22=N2-N21;
	
	
	r1=qt(p=1-alpha_local/2, df=N1-2);
	for(index in 1:iter_inside){
		
		#stage_1
		seq1=rnorm(n=N11, mean=Period_dif+delta_nomial, sd=sqrt(sigma_nomial));
		seq2=rnorm(n=N12, mean=Period_dif-delta_nomial, sd=sqrt(sigma_nomial));
		delta_hat=mean(seq1)-mean(seq2);
		sigma_s_hat=(sum((seq1-mean(seq1))^2)+sum((seq2-mean(seq2))^2) )/(N1-2);
		xi=sqrt(sigma_s_hat*(1/N11+1/N12)/4);
		
		Z=(delta_hat-delta_nomial)/xi;
		a_L1=r1+L/xi;	a_U1=-a_L1;
		r_L1=min(a_L1, -r1+Psi*L/xi);
		r_U1=max(a_U1, r1+Psi*U/xi);
		
		if(Z<a_U1 & Z>a_L1){true_counter=true_counter+1;}
		if(Z<r_U1 & Z>r_L1){
			#stage_2
			r1=qnorm(p=1-alpha_local/2);
			seq1=c(seq1, rnorm(n=N21, mean=Period_dif+delta_nomial, sd=sqrt(sigma_nomial)) );
			seq2=c(seq2, rnorm(n=N22, mean=Period_dif-delta_nomial, sd=sqrt(sigma_nomial)) );
			delta_hat=mean(seq1)-mean(seq2);
			sigma_s_hat=(sum((seq1-mean(seq1))^2)+sum((seq2-mean(seq2))^2) )/(N1+N2-2);
			xi=sqrt(sigma_s_hat*(1/N21+1/N22)/4);
			a_L2=r1+L/xi;	a_U2=-a_L2;
			Z=(delta_hat-delta_nomial)/xi;
			if(Z<a_U2 & Z>a_L2){true_counter=true_counter+1;}
		}
	}
	return(true_counter/iter_inside);
}
...
CP50<-function(t=0.5, beta_loc=beta, iter=5000, original_sample_size=120){
	...
	X=Int_Psi_Alg(CP, lambda_coef=delta_hat)
