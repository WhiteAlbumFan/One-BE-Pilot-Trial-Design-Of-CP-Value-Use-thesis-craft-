...
Psi_Simulation_Sqrt<-function(iter=5000, Max=200, N=12){
...
#		r_L1=min(a_L1, (Psi_l*L-delta_nomial)/sqrt( sigma_nomial/time1 ) );
		...
		return(integrate( function(x){
						...
						lower=(n_star-2)^2*1/(qchisq(p=0.8, df=n_star-2)), upper=n_star*2-4)$value );
	}
...
#		r_L1=min(a_L1, (Psi_l*L-delta_nomial)/sqrt( sigma_nomial/time1 ) );
	...
	while(sample_size_Psi(N0, Psi_l = Psi)<=0.8 & N0<100){
		N0=N0+2;
	}
	print(N0);
	N0=max(N0, N);
	...
	N1=as.integer(N0*pilot_size);	N11=as.integer(N1*pilot_dtr);	N12=N1-N11;
	N2=N0;							N21=as.integer(N2*pivotal_dtr);	N22=N2-N21;
...
Psi_Simulation_Const(Psi=0.5, N=12, beta1=0.5, beta2=0.8)

U^2/(4*sigma_nomial^2*qnorm(1-alpha/2)^2)

Psi_Simulation_Gould<-function(iter=5000, Psi=0.5, Max=200, N=12){
...
#		r_L1=min(a_L1, (Psi_l*L-delta_nomial)/sqrt( sigma_nomial/time1 ) );
	...
	N0=12;
	while(sample_size_Psi(N0)<=0.8 & N0<100){
		N0=N0+2;
	}
	print(N0);
...
test_tears<-function(n=12){
	#C=0.5;
	#print(pchisq(q=C*n, df=n) );
	#print(pchisq(q=n/C, df=n) );
	qchisq(p=0.95, df=n)/n;
}

test_tears
plot(test_tears, c(100))
